...
export function FormStat(props) {
  const data = {
    labels: ["Posted", "Submitted"],
    datasets: [
      {
        label: "# Posted vs Submitted ",
        data: [props.posted, props.submitted],
  ...
  return (
    <div className="card m-7 w-min border border-gray-800 bg-gray-800 text-primary-content shadow-inner  hover:border-primary  hover:shadow-primary">
      <div className="m=5 card-body flex w-64 self-center">
        <h2 className="card-title ">Forms Posted Vs Forms Submitted Chart</h2>
        <Doughnut data={data} />
      </div>
    </div>
  );
